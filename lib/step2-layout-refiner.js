  ...
  for (let attemptNumber = 1; attemptNumber <= maxAttempts; attemptNumber += 1) {
    ...
    attemptTrace.judge = {
      input: {
        ...
        sourcePdfBytes: estimateBase64Bytes(sourcePdfBase64),
        sourceFileName,
        localIssues,
      },
      output: {
...
async function judgeRenderedSlideLayout({
  ...
  const output = await generateText({
    ...
    parts: [
      {
        text: `添付1: 元論文PDF（${sourceFileName || "paper.pdf"}）。スライド内容が論文から外れていないか確認するための一次資料です。`,
      },
      {
        inline_data: {
      ...
      {
        text: "添付2: 現在のMarp Markdownから実際にレンダリングしたスライドPDF。主にレイアウト、数式表示、見切れ、余白を確認してください。",
      },
      {
        inline_data: {
          mime_type: "application/pdf",
...
async function generateCorrectedStep2Markdown({
  ...
  const output = await generateText({
    ...
    parts: [
      {
        text: "添付1: 現在のMarp Markdownから実際にレンダリングしたスライドPDF。修正すべきレイアウト問題を確認してください。",
      },
      {
        inline_data: {
          mime_type: "application/pdf",
      ...
      {
        text: `添付2: 元論文PDF（${sourceFileName || "paper.pdf"}）。内容を削りすぎたり、論文外の主張を足したりしないための一次資料です。`,
      },
      {
        inline_data: {
          mime_type: "application/pdf",
